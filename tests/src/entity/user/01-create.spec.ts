...
test.describe(UserDocs.create.operation.summary, () => {
  ...
  test(`Buscar Usuário - ${UserDocs.findOne.operation.summary} - retorna o user criado`, async () => {
    ...
    const { error, response } = await authedClient(user.id, user.username)
      .safeCommand<{ id: string; username: string; email: string }, { id: string }>({
  ...
  test(`Buscar Usuário Inexistente - ${UserDocs.findOne.operation.summary} - ${UserDocs.findOne.responses![0].description}`, async () => {
